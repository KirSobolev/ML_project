...
# Initialize LIME exaplainer to explain forecasts later
...
# Add Calendar and a Frame to hold the calendar and button
calendar_frame = tk.Frame(root)
calendar_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nw")
# Frame for data labels
...
def select_day_df(date, df=df):
    """
    Returns pd.Dataframe of selected day
    """
...
def create_plot(df, predictions):
    """
    Creates a plot with comparison of prediction and real value
    """
...
def on_confirm():
    """
    Main function that produces predictictions, slider, data lables
    """
    ...
    def on_slider_change(val):
        """
        On slider change prints out new data values and labels
        """
        index = int(float(val))
        ...
        # LIME explainer that tries to explane models' decision
